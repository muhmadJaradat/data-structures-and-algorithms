...
const listFoods = (recipe) => {
  ...
  result=recipe.ingredients.slice(0,recipe.ingredients.length).map(element=>{
    return element.split(' ').slice(2).join(' ');
...
const stepActions = (recipe) => {
  let result = [];
  // Solution code here...
  result=recipe.steps.slice(0,recipe.ingredients.length).map(element=>{
    return element.split(' ').slice(0,1).join(' ');
...
const removeLastCharacters = (str, numberOfCharacters) => {
  // Solution code here...
  let newStr='';
  numberOfCharacters < 0?newStr=str:numberOfCharacters>=str.length?'':newStr=str.slice(0,str.length-numberOfCharacters);
  return newStr;
};
...
const totalSumCSV = (str) => {
  ...
  str.split(',').map(element=>{
    return element!==','?total+=Number(element):null;
  });
  return total;
};
...
const extractVowels = (str) => {
  ...
  let newArr=[];
  let newStr='';
  newArr.push(str.replace(/[aeiou]/gi, ''));
  for (let i = 0; i < str.length; i++) {
    str[i]==='a'||str[i]==='e'||str[i]==='i'||str[i]==='o'||str[i]==='u'?newStr+=str[i]:null;

  }
  newArr.push(newStr.split('').sort().join(''));
  return newArr;
